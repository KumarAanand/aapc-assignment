...
import tempHighLow from '~/components/TempHighLow';
...
function TempHighLow({ weather }: { weather: WeatherData[] }) {
  if (!weather.length) return null;

  const highest = weather.reduce((prev, current) =>
    current.temperature?.celsius > prev.temperature?.celsius ? current : prev
  );

  const lowest = weather.reduce((prev, current) =>
    current.temperature?.celsius < prev.temperature?.celsius ? current : prev
  );

  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-4 hover:shadow-lg transition-shadow">
      <h2 className="text-xl font-semibold mb-4">Temperature Overview</h2>
      <div className="space-y-2">
        <p className="text-lg">
          <strong>Highest Temperature:</strong> {highest.city} - {highest.temperature?.celsius?.toFixed(1)}°C
          &nbsp;&nbsp;&nbsp;<strong>Lowest Temperature:</strong> {lowest.city} - {lowest.temperature?.celsius?.toFixed(1)}°C
        </p>
      </div>
    </div>
  );
...
export default function Home() {
  ...
  return (
    <div>
      ...
      {weather.length > 0 && <TempHighLow weather={weather} />}
